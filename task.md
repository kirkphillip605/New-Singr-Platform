...
## Deliverables
- [/] WebSocket Server (`apps/api-node/src/ws/ws-server.ts`)
  - [ ] Initialize socket.io server integrated with Express HTTP server
  - [ ] Session authentication middleware validating connection cookies
  - [ ] Dynamic room joining (`show:<showId>`) for singers and hosts
  - [ ] Implement ping-pong heartbeats and disconnection handling
- [/] BullMQ Workers (`apps/api-node/src/workers/`)
  - [ ] Install `bullmq` dependency
  - [ ] Configure `song-sync.queue.ts` with 5s debounce reset logic
  - [ ] Create `song-sync.worker.ts` migrating C++ shadow-swap debounce to Redis
  - [ ] Emit socket.io updates when song sync processes are completed

## Verification
- [ ] WebSocket client connects, authenticates, and joins show rooms successfully
- [ ] Singer REST submit requests emit `new_request` WebSocket events
- [ ] Debounced BullMQ queue correctly replaces C++ memory timeouts
- [ ] Swapping live songs successfully updates clients in real-time
